...
print('Starting... (ID: {0}, Username: {1})'.format(
    ...
    if update.message.from_user.id == CONFIG['Admin']:
        if update.message.reply_to_message:
            if message_list.__contains__(str(update.message.reply_to_message.message_id)):
                sender_id = message_list[str(
                    update.message.reply_to_message.message_id)]['sender_id']
                try:
                    if update.message.audio:
                        bot.send_audio(
                            chat_id=sender_id, audio=update.message.audio, caption=update.message.caption)
                    elif update.message.document:
                        bot.send_document(
                            chat_id=sender_id, document=update.message.document, caption=update.message.caption)
                    elif update.message.voice:
                        bot.send_voice(
                            chat_id=sender_id, voice=update.message.voice, caption=update.message.caption)
                    elif update.message.video:
                        bot.send_video(
                            chat_id=sender_id, video=update.message.video, caption=update.message.caption)
                    elif update.message.sticker:
                        bot.send_sticker(chat_id=sender_id,
                                         sticker=update.message.sticker)
                    elif update.message.photo:
                        bot.send_photo(
                            chat_id=sender_id, photo=update.message.photo[0], caption=update.message.caption)
                    elif update.message.text_markdown:
                        bot.send_message(
                            chat_id=sender_id, text=update.message.text_markdown, parse_mode=telegram.ParseMode.MARKDOWN)
                    else:
                        bot.send_message(
                ...
                if preference_list[str(update.message.from_user.id)]['receipt']:
                    bot.send_message(chat_id=update.message.chat_id, text=LANG['reply_message_sent'] % (
                        preference_list[str(sender_id)]['name'], str(sender_id)), parse_mode=telegram.ParseMode.MARKDOWN)
            else:
                bot.send_message(
                    chat_id=CONFIG['Admin'], text=LANG['reply_to_message_no_data'])
        else:
            bot.send_message(
                chat_id=CONFIG['Admin'], text=LANG['reply_to_no_message'])
    else:
        # only when conversation is true
        if preference_list[str(update.message.from_user.id)]['conversation']:
            # forward messege to me
            msg_forward_to_me = bot.forward_message(
                chat_id=CONFIG['Admin'], from_chat_id=update.message.chat_id, message_id=update.message.message_id)
            # add user-info output to me when content is sticker or photo.
            if update.message.sticker or update.message.photo:
                bot.send_message(chat_id=update.message.chat_id, text=LANG['info_data'] % (
                    preference_list[str(sender_id)]['name'], str(sender_id)), parse_mode=telegram.ParseMode.MARKDOWN)

            # receipt to the sender
            if preference_list[str(update.message.from_user.id)]['receipt']:
                bot.send_message(chat_id=update.message.from_user.id,
                                 text=LANG['message_received_receipt'])
...
def process_command(bot, update):
    init_user(update.message.from_user)
    global CONFIG
    command = update.message.text[1:].replace(
        CONFIG['Username'], '').lower().split()
    if command[0] == 'start':
        ...
        global preference_list
        preference_list[str(update.message.from_user.id)]['receipt'] = (
            preference_list[str(update.message.from_user.id)]['receipt'] == False)
        threading.Thread(target=save_preference).start()
        if preference_list[str(update.message.from_user.id)]['receipt']:
            bot.send_message(chat_id=update.message.chat_id,
                             text=LANG['receipt_on'])
        else:
            bot.send_message(chat_id=update.message.chat_id,
                             text=LANG['receipt_off'])
    ...
    # start conversation
        ...
        global preference_list
        preference_list[str(update.message.from_user.id)
                        ]['conversation'] = True
    # end conversation
    elif command[0] == 'done':
        global preference_list
...
dispatcher.add_handler(telegram.ext.MessageHandler(telegram.ext.Filters.all & telegram.ext.Filters.private & (
    ~ telegram.ext.Filters.command) & (~ telegram.ext.Filters.status_update), process_msg))

dispatcher.add_handler(telegram.ext.MessageHandler(
    telegram.ext.Filters.command & telegram.ext.Filters.private, process_command))
