...
def process_command(bot, update):
	...
	if not preference_list[str(idf_fromuser)]['blacklist'] or idf_fromuser == CONFIG['Admin'] :
		...
		##bot start
		if command[0] == 'start' :
			bot.send_message(chat_id=idf_chat, text=LANG['info_start'])
			return
		##start conversation
		elif command[0] == 'say' :
			preference_list[str(idf_fromuser)]['conversation'] = True
			bot.send_message(chat_id=idf_chat, text=LANG['notify_conversation_start'])
		##end conversation
		elif command[0] == 'done' :
			preference_list[str(idf_fromuser)]['conversation'] = False
			bot.send_message(chat_id=idf_chat, text=LANG['notify_conversation_end'])
		##messege-info you point
		elif command[0] == 'messege_info' :
			if (idf_fromuser == CONFIG['Admin']) and (idf_chat == CONFIG['Admin']):
				if update.message.reply_to_message:
					if message_list.__contains__(str(update.message.reply_to_message.message_id)):
						sender_id = message_list[str(update.message.reply_to_message.message_id)]['sender_id']
						bot.send_message(chat_id=idf_chat, text=LANG['receipt_admin_receive'] % (preference_list[str(sender_id)]['name'], str(sender_id)), parse_mode=telegram.ParseMode.MARKDOWN)
					else:
						bot.send_message(chat_id=idf_chat, text=LANG['error_reply_nodata'])
			else:
				bot.send_message(chat_id=idf_chat, text=LANG['warning_user_adminonly'])
			...
			if (idf_fromuser == CONFIG['Admin']) and (idf_chat == CONFIG['Admin']):
				if update.message.reply_to_message:
					if message_list.__contains__(str(update.message.reply_to_message.message_id)):
						...
						bot.send_message(chat_id=idf_chat, text=LANG['operation_block'])
					else:
						bot.send_message(chat_id=idf_chat, text=LANG['error_reply_nodata'])
			else:
				bot.send_message(chat_id=idf_chat, text=LANG['warning_user_adminonly'])
			...
			if (idf_fromuser == CONFIG['Admin']) and (idf_chat == CONFIG['Admin']):
				if update.message.reply_to_message:
					if message_list.__contains__(str(update.message.reply_to_message.message_id)):
						...
						bot.send_message(chat_id=idf_chat, text=LANG['operation_unblock'])
					else:
						bot.send_message(chat_id=idf_chat, text=LANG['error_reply_nodata'])
			else:
				bot.send_message(chat_id=idf_chat, text=LANG['warning_user_adminonly'])
		# only when 'conversation' false, can operate other bot directives, as to make /done useful
			...
			if command[0] == 'receipt_switch' :
				...
				threading.Thread(target=save_preference).start()
				if preference_list[str(idf_fromuser)]['receipt']:
					bot.send_message(chat_id=idf_chat, text=LANG['info_receipt_on'])
				else:
					bot.send_message(chat_id=idf_chat, text=LANG['info_receipt_off'])
			##bot version
			elif command[0] == 'version' :
				bot.send_message(chat_id=idf_chat, text='Telegram PM Bot to concatnate your conversation between the bot and the sender.\n\nhttps://github.com/NewBugger/telegram-pm-bot')
			else:
				##when received not existed command
				bot.send_message(chat_id=idf_chat, text=LANG['warning_user_commandnotfound'])
		# ask user to /done
		else:
			bot.send_message(chat_id=idf_chat, text=LANG['warning_conversation_end'])
	else:
		bot.send_message(chat_id=idf_chat, text=LANG['warning_user_blocked'])
